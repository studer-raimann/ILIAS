...
class ilObjQuestionPoolGUI extends ilObjectGUI
{
    ...
    public function __construct()
    {
        ...
        $this->authoring_service = $DIC->assessment()->questionAuthoring($this->object->getId(), $DIC->user()
            ->getId());
        // TODO
        $this->back_link = $DIC->ui()
            ->factory()
            ->link()
            ->standard('TODO', $DIC->ctrl()
            ->getLinkTarget($this));
    }
    ...
    function executeCommand()
    {
        ...
        switch ($next_class) {
            case "ilasqquestionauthoringgui":
                ...
                $authoring_gui = $this->authoring_service->question($this->authoring_service->currentOrNewQuestionId())
                    ->getAuthoringGUI($this->back_link, $this->object->getRefId(), $this->object->getType(), new \QuestionConfig(), $DIC->access()
            ...
            case "ilassquestionpagegui":
                ...
                $page_gui->obj->addUpdateListener($question, 'updateTimestamp');
                ...
                $page_gui->setQuestionHTML(array(
                    $q_gui->object->getId() => $q_gui->getPreview(TRUE)
                ));
            ...
            case 'ilassquestionhintsgui':
                ...
                $gui->setEditingEnabled($DIC->access()
                    ->checkAccess('write', '', $this->object->getRefId()));
            ...
            case 'ilquestionpoolskilladministrationgui':
                ...
                $gui = new ilQuestionPoolSkillAdministrationGUI($ilias, $ilCtrl, $ilAccess, $ilTabs, $tpl, $lng, $ilDB, $ilPluginAdmin, $this->object, $this->ref_id);
    ...
    {
        global $DIC; /* @var ILIAS\DI\Container $DIC */
        $enableComments = $DIC->rbac()
            ->system()
            ->checkAccess('write', $_GET['ref_id']);
    ...
    function questionsObject()
    {
        ...
        if ($rbacsystem->checkAccess('write', $_GET['ref_id'])) {
            ...
            $creationLinkComponent = $this->authoring_service->question($this->authoring_service->currentOrNewQuestionId())
                ->getCreationLink([
                'ilObjQuestionPoolGUI'
            ]);
    ...
    function getTabs()
    {
        ...
        $this->tabs_gui->addTarget("assQuestions", $this->ctrl->getLinkTarget($this, "questions"), array(
            "questions",
            "filter",
            "resetFilter",
            "createQuestion",
        ...
        if ($ilAccess->checkAccess("read", "", $this->ref_id) || $ilAccess->checkAccess("visible", "", $this->ref_id)) {
            $this->tabs_gui->addTarget("info_short", $this->ctrl->getLinkTarget($this, "infoScreen"), array(
                "infoScreen",
                "showSummary"
            ));
        }
        ...
        if ($ilAccess->checkAccess("write", "", $_GET['ref_id'])) {
            // properties
            $this->tabs_gui->addTarget('settings', $this->ctrl->getLinkTargetByClass('ilObjQuestionPoolSettingsGeneralGUI'), array(), array(
        ...
        // print view
        $this->tabs_gui->addTarget("print_view", $this->ctrl->getLinkTarget($this, 'print'), array(
            "print"
        ), "", "");
        ...
        if ($ilAccess->checkAccess("write", "", $this->object->getRefId())) {
            ...
            $mdtab = $mdgui->getTab();
            if ($mdtab) {
                $this->tabs_gui->addTarget("meta_data", $mdtab, "", "ilmdeditorgui");
            }
        ...
        if ($currentUserHasWriteAccess) {
            $this->tabs_gui->addTarget("export", $this->ctrl->getLinkTargetByClass("ilquestionpoolexportgui", ""), "", "ilquestionpoolexportgui");
        }

        if ($ilAccess->checkAccess("edit_permission", "", $this->object->getRefId())) {
            $this->tabs_gui->addTarget("perm_settings", $this->ctrl->getLinkTargetByClass(array(
                get_class($this),
                'ilpermissiongui'
            ), "perm"), array(
                "perm",
     ...
     * Redirect script to call a test with the question pool reference id
     *
     * @param integer $a_target
     *            The reference id of the question pool
     * @access public
     */
    ...
    private function buildQuestionBrowserTableGUI($taxIds)
    {
        ...
        foreach ($table_gui->getFilterItems() as $item) {
            if (substr($item->getPostVar(), 0, strlen('tax_')) == 'tax_') {
                ...
                $questionList->addTaxonomyFilter($taxId, $item->getValue(), $this->object->getId(), $this->object->getType());
        ...
        if ($this->object->isNavTaxonomyActive() && (int) $_GET['tax_node']) {
            ...
            if ((int) $_GET['tax_node'] != $rootNodeId) {
                $questionList->addTaxonomyFilter($this->object->getNavTaxonomyId(), array(
                    (int) $_GET['tax_node']
    ...
    public function buildAsqAuthoringSpecification(): AuthoringServiceSpecContract
    {
        ...
        // TODO
        $containerBackLink = $DIC->ui()
            ->factory()
            ->link()
            ->standard('Back to Question Pool', $DIC->ctrl()
            ->getLinkTarget($this, 'showQuestionList'));

        $authoringSpecification = $DIC->assessment()
            ->specification()
            ->authoring($this->object->getId(), $DIC->user()
            ->getId(), $containerBackLink);//->addAdditionalConfigSection($this->buildAdditionalTaxonomiesConfigSection());
