...
class ilObjTestGUI extends ilObjectGUI
{
    ...
    function executeCommand()
    {
        ...
        switch ($next_class) {
            ...
            case 'illtiproviderobjectsettinggui':
                ...
                $lti_gui->setCustomRolesForSelection($GLOBALS['DIC']->rbac()
                    ->review()
            ...
            case 'iltestdashboardgui':
                ...
                $gui = new ilTestDashboardGUI($this->object, $this->testQuestionSetConfigFactory->getQuestionSetConfig());
            ...
            case 'iltestresultsgui':
                ...
                $gui = new ilTestResultsGUI($this->object, $this->testQuestionSetConfigFactory->getQuestionSetConfig());
            ...
            case 'ilobjtestsettingsgeneralgui':
                ...
                $gui = new ilObjTestSettingsGeneralGUI($this->ctrl, $ilAccess, $this->lng, $this->tree, $ilDB, $ilPluginAdmin, $ilUser, $this);
                $this->ctrl->forwardCommand($gui);
                break;
            ...
            case 'ilobjtestsettingsscoringresultsgui':
                ...
                $gui = new ilObjTestSettingsScoringResultsGUI($this->ctrl, $ilAccess, $this->lng, $this->tree, $ilDB, $ilPluginAdmin, $this);
                $this->ctrl->forwardCommand($gui);
                break;
        ...
        if (! in_array(strtolower($_GET["baseClass"]), array(
            'iladministrationgui',
            'ilrepositorygui'
    ...
    protected function trackTestObjectReadEvent()
    {
        ...
        ilChangeEvent::_recordReadEvent($this->object->getType(), $this->object->getRefId(), $this->object->getId(), $DIC->user()->getId());
    }
    ...
    function addDidacticTemplateOptions(array &$a_options)
    {
        ...
        if (count($defaults)) {
            foreach ($defaults as $row) {
                $a_options["tstdef_" . $row["test_defaults_id"]] = array(
                    $row["name"],
        ...
        if ($templates) {
            foreach ($templates as $item) {
                $a_options["tsttpl_" . $item["id"]] = array(
                    $item["title"],
    ...
    public function confirmRemoveQuestionsObject()
    {
        ...
        if ($_REQUEST['test_express_mode']) {
            ...
            if (count($questions) == count($removeQuestionIds) || ! $return_to) {
    ...
    function questionsObject()
    {
        ...
        if ($ilAccess->checkAccess("write", "", $this->ref_id)) {
            ...
            if ($total != 0) {
                $link = $DIC->ui()
                    ...
                    ->link()
                    ->standard($DIC->language()
                    ->txt("test_has_datasets_warning_page_view_link"), $DIC->ctrl()
            ...
            if (count($messages) > 0) {
                $DIC->ui()
                    ->mainTemplate()
                    ->setCurrentBlock('mess');
                $DIC->ui()
                    ->mainTemplate()
                    ->setVariable('MESSAGE', implode("<br>", $messages));
                $DIC->ui()
                    ->mainTemplate()
                    ->parseCurrentBlock();
            } else {
                $DIC->ctrl()->setParameter($this, self::AUTHORING_CONTEXT_PARAMETER, self::AUTHORING_CONTEXT_LIST_VIEW);
        ...
        $table_gui = new ilTestQuestionsTableGUI($this, 'questions', $this->object->getRefId());

        $table_gui->setPositionInsertCommandsEnabled(is_array($_SESSION['tst_qst_move_' . $this->object->getTestId()]) && count($_SESSION['tst_qst_move_' . $this->object->getTestId()]));
    ...
    function initImportForm($a_new_type)
    {
        ...
        if (count($questionpools)) {
            $options = array(
                "-1" => $this->lng->txt("dont_use_questionpool")
            );
            foreach ($questionpools as $key => $value) {
    ...
    function infoScreen($session_lock = "")
    {
        ...
        if ($this->object->getShowInfo()) {
            ...
            if ($this->object->getNrOfTries() != 1) {
                $info->addProperty($this->lng->txt("tst_nr_of_tries_of_user"), ($toolbar->getTestSession()
                    ->getPass() == false) ? $this->lng->txt("tst_no_tries") : $toolbar->getTestSequence()
                    ->getPass());
            }
    ...
    protected function getTargetQuestionpoolForm($questionpools, $cmd)
    {
        ...
        $form->setFormAction($DIC->ctrl()
            ->getFormAction($this));
        $form->addCommandButton($cmd, $DIC->language()
            ->txt('submit'));
        $form->addCommandButton('cancelCreateQuestion', $DIC->language()
            ->txt('cancel'));
