...
class ilObjTestSettingsGeneralGUI extends ilTestSettingsGUI
{
     ...
     */
    public function __construct(ilCtrl $ctrl, ilAccessHandler $access, ilLanguage $lng, ilTree $tree, ilDBInterface $db, ilPluginAdmin $pluginAdmin, ilObjUser $activeUser, ilObjTestGUI $testGUI)
    {
    ...
    protected function getSettingsTemplateMessageHTML()
    {
        if ($this->settingsTemplate) {
            ...
            $msgHTML = ilUtil::getSystemMessageHTML(sprintf($this->lng->txt("test_using_template"), $title, $link), "info");
        ...
        if ($form->getItemByPostVar('online')->getChecked() == 1) {
            ...
            switch ($this->testOBJ->getQuestionSetType()) {
                case ilTestQuestionSetConfig::TYPE_FIXED:
                    $question_list = $DIC->assessment()
                        ->questionAuthoring($DIC->ctrl()
                        ->getContextObjId(), $DIC->user()
                        ->getId())
                        ->questionList();
            ...
            foreach ($DIC->assessment()
                ->questionAuthoring($DIC->ctrl()
                ->getContextObjId(), $DIC->user()
                ->getId())
                ->questionList()
                ->getQuestionsOfContainerAsDtoList(true) as $question_dto) {
    ...
    private function buildForm()
    {
        ...
        $orgunitServiceActive = ilOrgUnitGlobalSettings::getInstance()->getObjectPositionSettingsByType($this->testOBJ->getType())
            ->isActive();
    ...
    private function performSaveForm(ilPropertyFormGUI $form)
    {
        ...
        if (! $this->testOBJ->participantDataExist()) {
            ...
            if (ilObjTest::isSkillManagementGloballyActivated() && $form->getItemByPostVar('skill_service') instanceof ilFormPropertyGUI) {
                $this->testOBJ->setSkillServiceEnabled($form->getItemByPostVar('skill_service')
                    ->getChecked());
            }
        }
    ...
    private function addGeneralProperties(ilPropertyFormGUI $form)
    {
        ...
        $questSetTypeFixed = new ilRadioOption($this->lng->txt("tst_question_set_type_fixed"), ilTestQuestionSetConfig::TYPE_FIXED, $this->lng->txt("tst_question_set_type_fixed_desc"));
        $questSetType->addOption($questSetTypeFixed);
        $questSetTypeRandom = new ilRadioOption($this->lng->txt("tst_question_set_type_random"), ilTestQuestionSetConfig::TYPE_RANDOM, $this->lng->txt("tst_question_set_type_random_desc"));
        $questSetType->addOption($questSetTypeRandom);
        $questSetTypeContinues = new ilRadioOption($this->lng->txt("tst_question_set_type_dynamic"), ilTestQuestionSetConfig::TYPE_DYNAMIC, $this->lng->txt("tst_question_set_type_dynamic_desc"));
    ...
    private function saveGeneralProperties(ilPropertyFormGUI $form)
    {
        ...
        // title
        $md_section->setTitle(ilUtil::stripSlashes($form->getItemByPostVar('title')
            ->getValue()));
        $md_section->update();
        ...
        if ($md_desc_ids) {
            ...
            $md_desc->setDescription(ilUtil::stripSlashes($form->getItemByPostVar('description')
                ->getValue()));
            $md_desc->update();
        } else {
        ...
        $this->testOBJ->setTitle(ilUtil::stripSlashes($form->getItemByPostVar('title')
            ->getValue()));
        $this->testOBJ->setDescription(ilUtil::stripSlashes($form->getItemByPostVar('description')
            ->getValue()));

        $this->testOBJ->setOfflineStatus(! $form->getItemByPostVar('online')
            ->getChecked());
        ...
        if ($form->getItemByPostVar('use_pool') instanceof ilFormPropertyGUI) {
            $this->testOBJ->setPoolUsage((int) $form->getItemByPostVar('use_pool')
                ->getValue());
        }
        ...
        if (! $this->testOBJ->participantDataExist()) {
            // question set type
            if ($form->getItemByPostVar('question_set_type') instanceof ilFormPropertyGUI) {
                $this->testOBJ->setQuestionSetType($form->getItemByPostVar('question_set_type')
                    ->getValue());
            }
        }
        ...
        if (! $this->testOBJ->participantDataExist() && $this->formPropertyExists($form, 'anonymity')) {
            $this->testOBJ->setAnonymity($form->getItemByPostVar('anonymity')
                ->getValue());
        }
    }
    ...
    private function saveAvailabilityProperties(ilPropertyFormGUI $form)
    {
        ...
        if ($form->getItemByPostVar('activation_type')->getChecked()) {
            ...
            $this->testOBJ->setActivationVisibility($form->getItemByPostVar('activation_visibility')
                ->getChecked());

            $period = $form->getItemByPostVar("access_period");
            $this->testOBJ->setActivationStartingTime($period->getStart()
                ->get(IL_CAL_UNIX));
            $this->testOBJ->setActivationEndingTime($period->getEnd()
                ->get(IL_CAL_UNIX));
        } else {
    ...
    private function saveTestIntroProperties(ilPropertyFormGUI $form)
    {
        if ($form->getItemByPostVar('intro_enabled') instanceof ilFormPropertyGUI) {
            $this->testOBJ->setIntroductionEnabled($form->getItemByPostVar('intro_enabled')
                ->getChecked());

            if ($form->getItemByPostVar('intro_enabled')->getChecked()) {
                $this->testOBJ->setIntroduction($form->getItemByPostVar('introduction')
                    ->getValue(), false, ilObjAdvancedEditing::_getUsedHTMLTagsAsString("assessment"));
            } else {
        ...
        if ($form->getItemByPostVar('showinfo') instanceof ilFormPropertyGUI) {
            $this->testOBJ->setShowInfo($form->getItemByPostVar('showinfo')
                ->getChecked());
        }
    }
    ...
    private function saveTestAccessProperties(ilPropertyFormGUI $form)
    {
        ...
        if ($this->formPropertyExists($form, 'password_enabled')) {
            $this->testOBJ->setPasswordEnabled($form->getItemByPostVar('password_enabled')
                ->getChecked());

            if ($form->getItemByPostVar('password_enabled')->getChecked()) {
                $this->testOBJ->setPassword($form->getItemByPostVar('password')
                    ->getValue());
            } else {
        ...
        if ($this->formPropertyExists($form, 'fixedparticipants') && ! $this->testOBJ->participantDataExist()) {
            $this->testOBJ->setFixedParticipants($form->getItemByPostVar('fixedparticipants')
                ->getChecked());
        }

        if ($this->formPropertyExists($form, 'limitUsers')) {
            $this->testOBJ->setLimitUsersEnabled($form->getItemByPostVar('limitUsers')
                ->getChecked());

            if ($form->getItemByPostVar('limitUsers')->getChecked()) {
                $this->testOBJ->setAllowedUsers($form->getItemByPostVar('allowedUsers')
                    ->getValue());
                $this->testOBJ->setAllowedUsersTimeGap($form->getItemByPostVar('allowedUsersTimeGap')
                    ->getValue());
            } else {
    ...
    private function addTestRunProperties(ilPropertyFormGUI $form)
    {
        ...
        $blockAfterPassed = new ilCheckboxInputGUI($this->lng->txt('tst_block_passes_after_passed'), 'block_after_passed');
        ...
        if (! $this->testOBJ->participantDataExist()) {
            ...
            if ($form->getItemByPostVar('limitPasses') instanceof ilFormPropertyGUI) {
                if ($form->getItemByPostVar('limitPasses')->getChecked()) {
                    $this->testOBJ->setNrOfTries($form->getItemByPostVar('nr_of_tries')
                        ->getValue());

                    $this->testOBJ->setBlockPassesAfterPassedEnabled((bool) $form->getItemByPostVar('block_after_passed')
                        ->getChecked());
                } else {
            ...
            $this->testOBJ->setEnableProcessingTime($form->getItemByPostVar('chb_processing_time')
                ->getChecked());
            if ($this->testOBJ->getEnableProcessingTime()) {
                $this->testOBJ->setProcessingTimeByMinutes($form->getItemByPostVar('processing_time')
                    ->getValue());
                $this->testOBJ->setResetProcessingTime($form->getItemByPostVar('chb_reset_processing_time')
                    ->getChecked());
            } else {
    ...
    private function addQuestionBehaviourProperties(ilPropertyFormGUI $form)
    {
        ...
        $instant_feedback_contents->addOption(new ilCheckboxOption($this->lng->txt('tst_instant_feedback_results'), 'instant_feedback_points', $this->lng->txt('tst_instant_feedback_results_desc')));
        $instant_feedback_contents->addOption(new ilCheckboxOption($this->lng->txt('tst_instant_feedback_answer_generic'), 'instant_feedback_generic', $this->lng->txt('tst_instant_feedback_answer_generic_desc')));
        $instant_feedback_contents->addOption(new ilCheckboxOption($this->lng->txt('tst_instant_feedback_answer_specific'), 'instant_feedback_specific', $this->lng->txt('tst_instant_feedback_answer_specific_desc')));
        $instant_feedback_contents->addOption(new ilCheckboxOption($this->lng->txt('tst_instant_feedback_solution'), 'instant_feedback_solution', $this->lng->txt('tst_instant_feedback_solution_desc')));
        ...
        $instant_feedback_trigger = new ilRadioGroupInputGUI($this->lng->txt('tst_instant_feedback_trigger'), 'instant_feedback_trigger');
        $ifbTriggerOpt = new ilRadioOption($this->lng->txt('tst_instant_feedback_trigger_manual'), self::INSTANT_FEEDBACK_TRIGGER_MANUAL);
        ...
        $ifbTriggerOpt = new ilRadioOption($this->lng->txt('tst_instant_feedback_trigger_forced'), self::INSTANT_FEEDBACK_TRIGGER_FORCED);
        ...
        $answerFixation = new ilRadioGroupInputGUI($this->lng->txt('tst_answer_fixation_handling'), 'answer_fixation_handling');
        $radioOption = new ilRadioOption($this->lng->txt('tst_answer_fixation_none'), self::ANSWER_FIXATION_NONE);
        $radioOption->setInfo($this->lng->txt('tst_answer_fixation_none_desc'));
        $answerFixation->addOption($radioOption);
        $radioOption = new ilRadioOption($this->lng->txt('tst_answer_fixation_on_instant_feedback'), self::ANSWER_FIXATION_ON_INSTANT_FEEDBACK);
        $radioOption->setInfo($this->lng->txt('tst_answer_fixation_on_instant_feedback_desc'));
        $answerFixation->addOption($radioOption);
        $radioOption = new ilRadioOption($this->lng->txt('tst_answer_fixation_on_followup_question'), self::ANSWER_FIXATION_ON_FOLLOWUP_QUESTION);
        $radioOption->setInfo($this->lng->txt('tst_answer_fixation_on_followup_question_desc'));
        $answerFixation->addOption($radioOption);
        $radioOption = new ilRadioOption($this->lng->txt('tst_answer_fixation_on_instantfb_or_followupqst'), self::ANSWER_FIXATION_ON_IFB_OR_FUQST);
    ...
    private function saveQuestionBehaviourProperties(ilPropertyFormGUI $form)
    {
        if ($form->getItemByPostVar('title_output') instanceof ilFormPropertyGUI) {
            $this->testOBJ->setTitleOutput($form->getItemByPostVar('title_output')
                ->getValue());
        }

        if ($form->getItemByPostVar('autosave') instanceof ilFormPropertyGUI) {
            $this->testOBJ->setAutosave($form->getItemByPostVar('autosave')
                ->getChecked());
            $this->testOBJ->setAutosaveIval($form->getItemByPostVar('autosave_ival')
                ->getValue() * 1000);
        }

        if ($form->getItemByPostVar('chb_shuffle_questions') instanceof ilFormPropertyGUI) {
            $this->testOBJ->setShuffleQuestions($form->getItemByPostVar('chb_shuffle_questions')
                ->getChecked());
        }

        if (! $this->testOBJ->participantDataExist() && $this->formPropertyExists($form, 'offer_hints')) {
            $this->testOBJ->setOfferingQuestionHintsEnabled($form->getItemByPostVar('offer_hints')
                ->getChecked());
        }
        ...
        if (! $this->testOBJ->participantDataExist() && $this->formPropertyExists($form, 'instant_feedback_enabled')) {
            if ($form->getItemByPostVar('instant_feedback_enabled')->getChecked()) {
                if ($this->formPropertyExists($form, 'instant_feedback_contents')) {
                    $this->testOBJ->setInstantFeedbackOptionsByArray($form->getItemByPostVar('instant_feedback_contents')
                        ->getValue());
                }
                if ($this->formPropertyExists($form, 'instant_feedback_trigger')) {
                    $this->testOBJ->setForceInstantFeedbackEnabled((bool) $form->getItemByPostVar('instant_feedback_trigger')
                        ->getValue());
                }
            } else {
        ...
        if (! $this->testOBJ->participantDataExist() && $this->formPropertyExists($form, 'answer_fixation_handling')) {
            $this->setAnswerFixationSettingsByFormValue($form->getItemByPostVar('answer_fixation_handling')
                ->getValue());
        }

        if (! $this->testOBJ->participantDataExist() && $this->formPropertyExists($form, 'obligations_enabled')) {
            $this->testOBJ->setObligationsEnabled($form->getItemByPostVar('obligations_enabled')
                ->getChecked());
        }
        ...
        if ($this->isCharSelectorPropertyRequired()) {
            ...
            $this->testOBJ->setCharSelectorAvailability($char_selector->getConfig()
                ->getAvailability());
            $this->testOBJ->setCharSelectorDefinition($char_selector->getConfig()
                ->getDefinition());
        }
    }
    ...
    private function addTestSequenceProperties(ilPropertyFormGUI $form)
    {
        ...
        $postpone->addOption(new ilRadioOption($this->lng->txt("tst_postpone_off"), 0, $this->lng->txt("tst_postpone_off_desc")));
        $postpone->addOption(new ilRadioOption($this->lng->txt("tst_postpone_on"), 1, $this->lng->txt("tst_postpone_on_desc")));
    ...
    private function saveTestSequenceSettings(ilPropertyFormGUI $form)
    {
        ...
        if ($this->formPropertyExists($form, 'chb_show_cancel')) {
            $this->testOBJ->setShowCancel($form->getItemByPostVar('chb_show_cancel')
                ->getChecked());
        }

        if ($this->formPropertyExists($form, 'postpone')) {
            $this->testOBJ->setPostponingEnabled((bool) $form->getItemByPostVar('postpone')
                ->getValue());
        }

        $this->testOBJ->setListOfQuestions($form->getItemByPostVar('list_of_questions')
            ->getChecked());
        ...
        if ($this->formPropertyExists($form, 'chb_show_marker')) {
            $this->testOBJ->setShowMarker($form->getItemByPostVar('chb_show_marker')
                ->getChecked());
        }
    }
    ...
    private function addTestFinishProperties(ilPropertyFormGUI $form)
    {
        ...
        $radio_rm->setValue(in_array($redirection_mode, array(
            REDIRECT_ALWAYS,
            REDIRECT_KIOSK
        )) ? $redirection_mode : REDIRECT_ALWAYS);
    ...
    private function saveTestFinishProperties(ilPropertyFormGUI $form)
    {
        ...
        $this->testOBJ->setShowFinalStatement($form->getItemByPostVar('showfinalstatement')
            ->getChecked());
        $this->testOBJ->setFinalStatement($form->getItemByPostVar('finalstatement')
            ->getValue(), false, ilObjAdvancedEditing::_getUsedHTMLTagsAsString("assessment"));

        if ($form->getItemByPostVar('redirection_enabled')->getChecked()) {
            $this->testOBJ->setRedirectionMode($form->getItemByPostVar('redirection_mode')
                ->getValue());
        } else {
            $this->testOBJ->setRedirectionMode(REDIRECT_NONE);
        }
        if (strlen($form->getItemByPostVar('redirection_url')->getValue())) {
            $this->testOBJ->setRedirectionUrl($form->getItemByPostVar('redirection_url')
                ->getValue());
        } else {
        ...
        if ($this->formPropertyExists($form, 'sign_submission')) {
            $this->testOBJ->setSignSubmission($form->getItemByPostVar('sign_submission')
                ->getChecked());
        }

        if ($this->formPropertyExists($form, 'mailnotification') && $form->getItemByPostVar('mailnotification')->getChecked()) {
            $this->testOBJ->setMailNotification($form->getItemByPostVar('mailnotification_content')
                ->getValue());
            $this->testOBJ->setMailNotificationType($form->getItemByPostVar('mailnottype')
                ->getChecked());
        } else {
