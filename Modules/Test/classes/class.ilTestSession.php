...
class ilTestSession
{
    ...
    protected function activeIDExists($user_id, $test_id)
    {
        ...
        if ($GLOBALS['DIC']['ilUser']->getId() != ANONYMOUS_USER_ID) {
            $result = $ilDB->queryF("SELECT * FROM tst_active WHERE user_fi = %s AND test_fi = %s", array(
                'integer',
                'integer'
            ), array(
                $user_id,
    ...
    function increaseTestPass()
    {
        ...
        if (time() - $_SESSION[$this->active_id]['tst_last_increase_pass'] > 10) {
            ...
            $ilDB->update('tst_active', array(
                ...
                'last_finished_pass' => array(
                    'integer',
                    $this->getLastFinishedPass()
                ),
                'last_started_pass' => array(
                    'integer',
                    $this->getLastStartedPass()
                ),
                'objective_container' => array(
                    'integer',
    ...
    function loadTestSession($test_id, $user_id = "", $anonymous_id = "")
    {
        ...
        if (($GLOBALS['DIC']['ilUser']->getId() == ANONYMOUS_USER_ID) && $this->doesAccessCodeInSessionExists()) {
            $result = $ilDB->queryF("SELECT * FROM tst_active WHERE user_fi = %s AND test_fi = %s AND anonymous_id = %s", array(
                'integer',
                'integer',
                'text'
            ), array(
     ...
     *            The database id of the test session
     */
    public function loadFromDb($active_id)
    {
        global $DIC;
        $ilDB = $DIC['ilDB'];
        $result = $ilDB->queryF("SELECT * FROM tst_active WHERE active_id = %s", array(
            'integer'
        ), array(
            $active_id
        ));
    ...
    public function lookupTestStartLock()
    {
        ...
        $res = $ilDB->queryF("SELECT start_lock FROM tst_active WHERE active_id = %s", array(
            'integer'
        ), array(
            $this->getActiveId()
        ));
