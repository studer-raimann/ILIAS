...
class ilTestEvaluationGUI extends ilTestServiceGUI
{
    ...
    public function __construct(ilObjTest $a_object)
    {
        ...
        $this->processLockerFactory = new ilTestProcessLockerFactory(new ilSetting('assessment'), $DIC->database());
    }
    ...
    function outEvaluation()
    {
        ...
        $participantData->setParticipantAccessFilter(ilTestParticipantAccessFilter::getAccessStatisticsUserFilter($this->ref_id));
        ...
        if (count($participantData->getActiveIds()) > 0) {
            ...
            if (! $this->object->getAnonymity()) {
                ...
                try {
                    $userCertificateRepository->fetchActiveCertificate($DIC->user()
                        ->getId(), $this->object->getId());
    ...
    function detailedEvaluation()
    {
        ...
        $this->object->setAccessFilteredParticipantList($this->object->buildStatisticsAccessFilteredParticipantList());

        $data = &$this->object->getCompleteEvaluationData();

        require_once 'Services/Form/classes/class.ilPropertyFormGUI.php';
        $form = new ilPropertyFormGUI();
        $form->setTitle(sprintf($this->lng->txt('detailed_evaluation_for'), $data->getParticipant($active_id)
            ->getName()));

        $resultPoints = new ilNonEditableValueGUI($this->lng->txt('tst_stat_result_resultspoints'));
        $resultPoints->setValue($data->getParticipant($active_id)
            ->getReached() . " " . strtolower($this->lng->txt("of")) . " " . $data->getParticipant($active_id)
            ->getMaxpoints() . " (" . sprintf("%2.2f", $data->getParticipant($active_id)
        ...
        if (strlen($data->getParticipant($active_id)->getMark())) {
            ...
            $resultMarks->setValue($data->getParticipant($active_id)
                ->getMark());
            ...
            if (strlen($data->getParticipant($active_id)->getECTSMark())) {
                ...
                $ectsGrade->setValue($data->getParticipant($active_id)
                    ->getECTSMark());
        ...
        if ($this->object->isOfferingQuestionHintsEnabled()) {
            ...
            $requestHints->setValue($data->getParticipant($active_id)
        ...
        $firstVisit->setValue(ilDatePresentation::formatDate(new ilDateTime($data->getParticipant($active_id)
            ->getFirstVisit(), IL_CAL_UNIX)));
        $form->addItem($firstVisit);

        $lastVisit = new ilNonEditableValueGUI($this->lng->txt('tst_stat_result_lastvisit'));
        $lastVisit->setValue(ilDatePresentation::formatDate(new ilDateTime($data->getParticipant($active_id)
            ->getLastVisit(), IL_CAL_UNIX)));
        $form->addItem($lastVisit);

        $nrPasses = new ilNonEditableValueGUI($this->lng->txt('tst_nr_of_passes'));
        $nrPasses->setValue($data->getParticipant($active_id)
            ->getLastPass() + 1);
        ...
        if ($this->object->getPassScoring() == SCORE_BEST_PASS) {
            $scoredPass->setValue($data->getParticipant($active_id)
                ->getBestPass() + 1);
        } else {
            $scoredPass->setValue($data->getParticipant($active_id)
                ->getLastPass() + 1);
        ...
        for ($pass = 0; $pass <= $data->getParticipant($active_id)->getLastPass(); $pass ++) {
            ...
            if ($finishdate > 0) {
                ...
                foreach ((array) $questions as $question) {
                    ...
                    $answeredquestion = $data->getParticipant($active_id)
                        ->getPass($pass)
    ...
    public function exportFileUploadsForAllParticipants()
    {
        ...
        if ($question_object instanceof ilObjFileHandlingQuestionType) {
            $question_object->deliverFileUploadZIPFile($this->ref_id, $this->object->getTestId(), $this->object->getTitle());
        } else {
    ...
    function eval_a()
    {
        ...
        if (count($foundParticipants)) {
            ...
            $average_time = $this->object->evalTotalStartedAverageTime($eval->getParticipantIds());
    ...
    public function exportEvaluation()
    {
        ...
        switch ($_POST["export_type"]) {
            case "excel":
                $expFactory->getExporter('results')->exportToExcel($deliver = TRUE, $filterby, $filtertext, $passedonly);
                break;

            case "csv":
                $expFactory->getExporter('results')->exportToCSV($deliver = TRUE, $filterby, $filtertext, $passedonly);
                break;
    ...
    public function exportCertificate()
    {
        ...
        $zipAction = new ilUserCertificateZip($objectId, $pathFactory->create($this->object));
        ...
        if (count($total_users)) {
            ...
            foreach ($total_users as $active_id => $name) {
                ...
                $pdfAction = new ilCertificatePdfAction($logger, $pdfGenerator, new ilCertificateUtilHelper(), $this->lng->txt('error_creating_certificate_pdf'));
     ...
     * If a question id and the id of the
    ...
    function outParticipantsPassDetails()
    {
        ...
        if (isset($_GET['statistics']) && $_GET['statistics'] == 1) {
            ...
            $ilTabs->setBackTarget($this->lng->txt('back'), $this->ctrl->getLinkTargetByClass('ilTestEvaluationGUI', 'detailedEvaluation'));
        } elseif ($this->object->getNrOfTries() == 1) {
            $ilTabs->setBackTarget($this->lng->txt('back'), $this->ctrl->getLinkTargetByClass('ilParticipantsTestResultsGUI'));
        } else {
            $ilTabs->setBackTarget($this->lng->txt('tst_results_back_overview'), $this->ctrl->getLinkTarget($this, 'outParticipantsResultsOverview'));
        }
        ...
        $result_array = $this->object->getTestResult($active_id, $pass, false, ! $this->getObjectiveOrientedContainer()
    ...
    function outUserPassDetails()
    {
        ...
        if (! $this->isPdfDeliveryRequest()) {
            ...
            if ($this->object->isShowExamIdInTestResultsEnabled()) {
                ...
                $tpl->setVariable('EXAM_ID', ilObjTest::lookupExamId($testSession->getActiveId(), $pass));
                $tpl->setVariable('EXAM_ID_TXT', $this->lng->txt('exam_id'));
                $tpl->parseCurrentBlock();
            }
        }

        if (! $this->getObjectiveOrientedContainer()->isObjectiveOrientedPresentationRequired() && $this->isGradingMessageRequired() && $this->object->getNrOfTries() == 1) {
            $gradingMessageBuilder = $this->getGradingMessageBuilder($active_id);
        ...
        $overviewTableGUI = $this->getPassDetailsOverviewTableGUI($result_array, $active_id, $pass, $this, "outUserPassDetails", $command_solution_details, $questionAnchorNav, $objectivesList);
    ...
    function outUserResultsOverview()
    {
        ...
        if ($this->getObjectiveOrientedContainer()->isObjectiveOrientedPresentationRequired()) {
            ...
            $loStatus->setCrsObjId($this->getObjectiveOrientedContainer()
                ->getObjId());
    ...
    function outUserListOfAnswerPasses()
    {
        ...
        if (! is_null($pass) && $this->object->isShowExamIdInTestResultsEnabled()) {
            ...
            $template->setVariable('EXAM_ID_VAL', ilObjTest::lookupExamId($testSession->getActiveId(), $pass));
    ...
    public function performDeletePass()
    {
        ...
        if ($pass == 0 && (($lastFinishedPass == 0 && $tries == 1 && $tries != $row['pass']) || ($isActivePass == true) // should be equal to || ($lastFinishedPass == -1 && $tries == 0)
        )) {
            $last_pass = true;
        } else {
            $last_pass = false;
    ...
    public function finishTestPassForSingleUser()
    {
        ...
        $cgui->setHeaderText(sprintf($this->lng->txt("finish_pass_for_user_confirmation"), $participantData->getFormatedFullnameByActiveId($activeId)));
