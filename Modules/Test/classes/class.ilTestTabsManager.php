...
class ilTestTabsManager
{
    ...
    {
        return $this->access->checkAccess('read', '', $this->getTestOBJ()
            ->getRefId());
    }
    ...
    {
        return $this->access->checkAccess('write', '', $this->getTestOBJ()
            ->getRefId());
    }
    ...
    {
        return $this->access->checkAccess('tst_statistics', '', $this->getTestOBJ()
            ->getRefId());
    }
    ...
    {
        return $this->access->checkAccess('edit_permission', '', $this->getTestOBJ()
            ->getRefId());
    }
    ...
    protected function setupTabsGuiConfig()
    {
        ...
        switch ($DIC->ctrl()->getCmd()) {
            ...
            case "searchParticipants":
                if ($this->isWriteAccessGranted() && in_array($DIC->ctrl()->getCmdClass(), array(
                    'ilobjtestgui',
                    'ilcertificategui'
                ))) {
                    $this->getSettingsSubTabs();
        ...
        if ($this->isWriteAccessGranted() && ! $this->isHiddenTab('assQuestions')) {
            ...
            switch ($this->getTestOBJ()->getQuestionSetType()) {
                case ilTestQuestionSetConfig::TYPE_FIXED:
                    $target = $DIC->ctrl()->getLinkTargetByClass('ilTestExpresspageObjectGUI', 'showPage');

                    // //////////////////////////////////////////////////////////
                    // TODO: remove when page view works with asq
                    $target = $DIC->ctrl()->getLinkTargetByClass('ilObjTestGUI', 'questions');
                    // //////////////////////////////////////////////////////////
            ...
            $this->tabs->addTab(self::TAB_ID_QUESTIONS, $DIC->language()
                ->txt('assQuestions'), $target);
        }

        // info tab
        if ($this->isReadAccessGranted() && ! $this->isHiddenTab('info_short')) {
            $this->tabs->addTarget("info_short", $DIC->ctrl()
                ->getLinkTargetByClass('ilObjTestGUI', 'infoScreen'), array(
                "infoScreen",
                "outIntroductionPage",
                "showSummary",
                "setAnonymousId",
        ...
        if ($this->isWriteAccessGranted()) {
            if (! $this->isHiddenTab('settings')) {
                ...
                $this->tabs->addTarget("settings", $DIC->ctrl()
                    ->getLinkTargetByClass('ilObjTestSettingsGeneralGUI'), $settingsCommands, array(
                    "ilmarkschemagui",
        ...
        if ($this->needsDashboardTab()) {
            $this->tabs->addTab(self::TAB_ID_EXAM_DASHBOARD, $DIC->language()
                ->txt('dashboard_tab'), $this->getDashboardTabTarget());
        }

        if ($this->needsResultsTab()) {
            $this->tabs->addTab(self::TAB_ID_RESULTS, $DIC->language()
                ->txt('results_tab'), $this->getResultsTabTarget());
        }

        if ($this->isLpAccessGranted() && ! $this->isHiddenTab(self::TAB_ID_LEARNING_PROGRESS)) {
            $this->tabs->addTarget(self::TAB_ID_LEARNING_PROGRESS, $DIC->ctrl()
        ...
        if ($this->checkScoreParticipantsTabAccess() && ! $this->isHiddenTab(self::TAB_ID_MANUAL_SCORING)) {
            ...
            if (count($scoring)) {
                // scoring tab
                $this->tabs->addTarget(self::TAB_ID_MANUAL_SCORING, $DIC->ctrl()
                    ->getLinkTargetByClass('ilTestScoringByQuestionsGUI', 'showManScoringByQuestionParticipantsTable'), array(
        ...
        if ($this->isWriteAccessGranted() && $scoring_adjust_active && ! $this->isHiddenTab(self::TAB_ID_CORRECTION)) {
            $this->tabs->addTab(self::TAB_ID_CORRECTION, $DIC->language()
                ->txt(self::TAB_ID_CORRECTION), $DIC->ctrl()
                ->getLinkTargetByClass('ilTestCorrectionsGUI'));
        }

        if ($this->checkStatisticsTabAccess() && ! $this->isHiddenTab(self::TAB_ID_STATISTICS)) {
            // statistics tab
            $this->tabs->addTarget(self::TAB_ID_STATISTICS, $DIC->ctrl()
                ->getLinkTargetByClass("iltestevaluationgui", "outEvaluation"), array(
                "statistics",
                "outEvaluation",
                "exportEvaluation",
                "detailedEvaluation",
        ...
        if ($this->isWriteAccessGranted()) {
            if (! $this->isHiddenTab(self::TAB_ID_HISTORY)) {
                ...
                // history
                $this->tabs->addTarget(self::TAB_ID_HISTORY, $DIC->ctrl()
                    ->getLinkTargetByClass('ilObjTestGUI', 'history'), "history", "");
            }
            ...
            if (! $this->isHiddenTab(self::TAB_ID_META_DATA)) {
                ...
                $mdtab = $mdgui->getTab();
                if ($mdtab) {
                    $this->tabs->addTarget(self::TAB_ID_META_DATA, $mdtab, "", "ilmdeditorgui");
                }
            }

            if (! $this->isHiddenTab(self::TAB_ID_EXPORT)) {
                // export tab
                $this->tabs->addTarget(self::TAB_ID_EXPORT, $DIC->ctrl()
                    ->getLinkTargetByClass('iltestexportgui', ''), '', array(
                    'iltestexportgui'
                ));
            }
        }
        ...
        if ($this->isPermissionsAccessGranted() && ! $this->isHiddenTab(self::TAB_ID_PERMISSIONS)) {
            $this->tabs->addTarget(self::TAB_ID_PERMISSIONS, $DIC->ctrl()
                ->getLinkTargetByClass(array(
                'ilObjTestGUI',
                'ilpermissiongui'
            ), "perm"), array(
    ...
    protected function getBrowseForQuestionsTab()
    {
        ...
        if ($this->isWriteAccessGranted()) {
            $DIC->ctrl()->saveParameterByClass($DIC->ctrl()
                ->getCmdClass(), 'q_id');
            // edit page
            $this->tabs->setBackTarget($this->lng->txt("backtocallingtest"), $DIC->ctrl()
                ->getLinkTargetByClass($DIC->ctrl()
                ->getCmdClass(), "questions"));
            $this->tabs->addTarget("tst_browse_for_questions", $DIC->ctrl()
                ->getLinkTargetByClass('ilObjTestGUI', "browseForQuestions"), array(
                "browseForQuestions",
                "filter",
                "resetFilter",
                "resetTextFilter",
    ...
    protected function getRandomQuestionsTab()
    {
        ...
        if ($this->isWriteAccessGranted()) {
            // edit page
            $this->tabs->setBackTarget($this->lng->txt("backtocallingtest"), $DIC->ctrl()
                ->getLinkTargetByClass('ilObjTestGUI', "questions"));
            $this->tabs->addTarget("random_selection", $DIC->ctrl()
                ->getLinkTargetByClass('ilObjTestGUI', "randomQuestions"), array(
                "randomQuestions"
            ), "", "");
        }
    }
    ...
    public function getQuestionsSubTabs()
    {
        ...
        if (! $this->getTestOBJ()->isRandomTest()) {
            $this->tabs->addSubTab(self::SUBTAB_ID_EXPRESSPAGE, $DIC->language()
                ->txt('questions_per_page_view'), $DIC->ctrl()
        ...
        if (! $this->isHiddenTab(self::SUBTAB_ID_QUESTIONS)) {
            $this->tabs->addSubTab(self::SUBTAB_ID_QUESTIONS, $DIC->language()
                ->txt('edit_test_questions'), $DIC->ctrl()
                ->getLinkTargetByClass('ilObjTestGUI', 'questions'));
        }

        // print view subtab
        if (! $this->getTestOBJ()->isRandomTest()) {
            $this->tabs->addSubTab(self::SUBTAB_ID_PRINTVIEW, $DIC->language()
                ->txt('print_view'), $DIC->ctrl()
                ->getLinkTargetByClass('ilObjTestGUI', 'print'));

            $this->tabs->addSubTab(self::SUBTAB_ID_REVIEW, $DIC->language()
                ->txt('review_view'), $DIC->ctrl()
                ->getLinkTargetByClass('ilObjTestGUI', 'review'));
        }
    }
    ...
    protected function getStatisticsSubTabs()
    {
        ...
        // user results subtab
        $this->tabs->addSubTabTarget("eval_all_users", $DIC->ctrl()
            ->getLinkTargetByClass("iltestevaluationgui", "outEvaluation"), array(
            "outEvaluation",
            "detailedEvaluation",
            "exportEvaluation",
        ...
        $this->tabs->addSubTabTarget("tst_results_aggregated", $DIC->ctrl()
            ->getLinkTargetByClass("iltestevaluationgui", "eval_a"), array(
            "eval_a"
        ), "", "");

        // question export
        $this->tabs->addSubTabTarget("tst_single_results", $DIC->ctrl()
            ->getLinkTargetByClass("iltestevaluationgui", "singleResults"), array(
            "singleResults"
        ), "", "");
    }
    ...
    public function getSettingsSubTabs()
    {
        ...
        // general subtab
        $this->tabs->addSubTabTarget('general', $DIC->ctrl()
            ->getLinkTargetByClass('ilObjTestSettingsGeneralGUI'), '', // auto activation regardless from cmd
        array(
            'ilobjtestsettingsgeneralgui'
        ) // auto activation for ilObjTestSettingsGeneralGUI
        );

        if (! $this->isHiddenTab('mark_schema')) {
            $this->tabs->addSubTabTarget('mark_schema', $DIC->ctrl()
                ->getLinkTargetByClass('ilmarkschemagui', 'showMarkSchema'), '', array(
                'ilmarkschemagui'
            ));
        }

        // scoring subtab
        $this->tabs->addSubTabTarget('scoring', $DIC->ctrl()
            ->getLinkTargetByClass('ilObjTestSettingsScoringResultsGUI'), '', // auto activation regardless from cmd
        array(
            'ilobjtestsettingsscoringresultsgui'
        ) // auto activation for ilObjTestSettingsScoringResultsGUI
        );

        // certificate subtab
        $validator = new ilCertificateActiveValidator();
        if (! $this->isHiddenTab('certificate') && true === $validator->validate()) {
            $this->tabs->addSubTabTarget("certificate", $DIC->ctrl()
                ->getLinkTargetByClass('ilObjTestGUI', 'certificate'), array(
                "certificate",
                "certificateEditor",
                "certificateRemoveBackground",
                "certificateSave",
        ...
        if (! $this->isHiddenTab('defaults')) {
            // defaults subtab
            $this->tabs->addSubTabTarget("tst_default_settings", $DIC->ctrl()
                ->getLinkTargetByClass('ilObjTestGUI', "defaults"), array(
                "defaults",
                "deleteDefaults",
                "addDefaults",
                "applyDefaults"
        ...
        if ($lti_settings->hasSettingsAccess()) {
            $this->tabs->addSubTabTarget('lti_provider', $DIC->ctrl()
                ->getLinkTargetByClass(ilLTIProviderObjectSettingGUI::class), '', [
    ...
    protected function getDashboardTabTarget()
    {
        ...
        return $DIC->ctrl()->getLinkTargetByClass(array(
            'ilTestDashboardGUI',
    ...
    public function getDashboardSubTabs()
    {
        ...
        $this->tabs->addSubTab(self::SUBTAB_ID_FIXED_PARTICIPANTS, $this->getDashbardParticipantsSubTabLabel(), $DIC->ctrl()
            ->getLinkTargetByClass('ilTestParticipantsGUI'));

        if ($this->needsTimeExtensionSubTab()) {
            $this->tabs->addSubTab(self::SUBTAB_ID_TIME_EXTENSION, $DIC->language()
                ->txt('timing'), $DIC->ctrl()
                ->getLinkTargetByClass('ilTestParticipantsTimeExtensionGUI'));
        }
    }
    ...
    public function needsMySolutionsSubTab()
    {
        ...
        return $this->getTestOBJ()->canShowSolutionPrintview($this->getTestSession()
            ->getUserId());
    }
    ...
    public function getResultsSubTabs()
    {
        ...
        if ($this->needsParticipantsResultsSubTab()) {
            $this->tabs->addSubTab(self::SUBTAB_ID_PARTICIPANTS_RESULTS, $DIC->language()
                ->txt('participants_results_subtab'), $DIC->ctrl()
                ->getLinkTargetByClass(array(
                'ilTestResultsGUI',
                'ilParticipantsTestResultsGUI'
            )));
        ...
        if ($this->needsLoResultsSubTab()) {
            $this->tabs->addSubTab(self::SUBTAB_ID_LO_RESULTS, $DIC->language()
                ->txt('tst_tab_results_objective_oriented'), $DIC->ctrl()
                ->getLinkTargetByClass(array(
                'ilTestResultsGUI',
                'ilTestEvalObjectiveOrientedGUI'
            )));
        ...
        if ($this->needsMyResultsSubTab()) {
            ...
            $this->tabs->addSubTab(self::SUBTAB_ID_MY_RESULTS, $myResultsLabel, $DIC->ctrl()
                ->getLinkTargetByClass(array(
                'ilTestResultsGUI',
                'ilMyTestResultsGUI',
                'ilTestEvaluationGUI'
        ...
        if ($this->needsSkillResultsSubTab()) {
            $this->tabs->addSubTab(self::SUBTAB_ID_SKILL_RESULTS, $DIC->language()
                ->txt('tst_show_comp_results'), $DIC->ctrl()
                ->getLinkTargetByClass(array(
                'ilTestResultsGUI',
        ...
        if ($this->needsHighSoreSubTab()) {
            $this->tabs->addSubTab(self::SUBTAB_ID_HIGHSCORE, $DIC->language()
                ->txt('tst_show_toplist'), $DIC->ctrl()
                ->getLinkTargetByClass(array(
                'ilTestResultsGUI',
                'ilTestToplistGUI'
            ), 'outResultsToplist'));
        ...
        if ($this->needsMySolutionsSubTab()) {
            $this->tabs->addSubTab(self::SUBTAB_ID_MY_SOLUTIONS, $DIC->language()
                ->txt('tst_list_of_answers_show'), $DIC->ctrl()
                ->getLinkTargetByClass(array(
                'ilTestResultsGUI',
