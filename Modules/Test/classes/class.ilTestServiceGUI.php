 ...
 * This class is the parent class for all
...
class ilTestServiceGUI
{
    ...
    public function getPassOverviewTableData(ilTestSession $testSession, $passes, $withResults)
    {
        ...
        $questionHintRequestRegister = ilAssQuestionHintTracking::getRequestRequestStatisticDataRegisterByActiveId($testSession->getActiveId());
    ...
    public function buildPassOverviewTableGUI($targetGUI)
    {
        ...
        $table->setPdfPresentationEnabled(isset($_GET['pdf']) && $_GET['pdf'] == 1);

        $table->setObjectiveOrientedPresentationEnabled($this->getObjectiveOrientedContainer()
     ...
     *            An array containing the results of the users test pass (generated by ilObjTest::getTestResult)
     * @param integer $active_id
     *            Active ID of the active user
     * @param integer $pass
     *            Test pass
     * @param boolean $show_solutions
     *            TRUE, if the solution output should be shown in the answers, FALSE otherwise
     ...
     *            TRUE if the anonymity status should be overwritten, FALSE otherwise
    ...
    {
        global $DIC;
        $asq_processing_service = $DIC->assessment()->questionProcessing($this->object->getId(), $DIC->user()
            ->getId(), $pass);
        ...
        $maxpoints = $asq_processing_service->userAnswer($revision_key)
            ->getUserAnswerScore()
            ->getMaxPoints();
        if ($maxpoints == 1) {
            $template->setVariable("QUESTION_TITLE", $this->object->getQuestionTitle($question_dto->getData()
                ->getTitle()) . " (" . $maxpoints . " " . $this->lng->txt("point") . ")");
        } else {
            $template->setVariable("QUESTION_TITLE", $this->object->getQuestionTitle($question_dto->getData()
                ->getTitle()) . " (" . $maxpoints . " " . $this->lng->txt("points") . ")");
        }
        if ($objectivesList !== null) {
            $objectives = $this->lng->txt('tst_res_lo_objectives_header') . ': ';
        ...
        $template->setVariable("SOLUTION_OUTPUT", $result_output);
        $template->setVariable("RECEIVED_POINTS", sprintf($this->lng->txt("you_received_a_of_b_points"), $asq_processing_service->userAnswer($revision_key)
            ->getUserAnswerScore()
            ->getReachedPoints(), $maxpoints));
    ...
    protected function getFilteredTestResult($active_id, $pass, $considerHiddenQuestions, $considerOptionalQuestions)
    {
        ...
        $questionList->setParentObjIdsFilter(array(
            $this->object->getId()
        ));
    ...
    protected function populateContent($content)
    {
        if ($this->isPdfDeliveryRequest()) {
            ...
            ilTestPDFGenerator::generatePDF($content, ilTestPDFGenerator::PDF_OUTPUT_DOWNLOAD, $this->object->getTitleFilenameCompliant(), PDF_USER_RESULT);
        } else {
